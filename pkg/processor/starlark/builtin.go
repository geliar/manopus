...
type builtinGetter func(context.Context) *starlark.Builtin

type builtin struct {
	ctx    context.Context
	getter map[string]builtinGetter
}

func NewBuiltIn(ctx context.Context, getter map[string]builtinGetter) starlark.Value {
	return &builtin{
		ctx:    ctx,
		getter: getter,
	}
...
func (m *builtin) Hash() (uint32, error) {
	return 0, errors.New("module is not hashable")
}
